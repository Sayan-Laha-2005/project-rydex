    ...
    html: `
        ...
        >
            ...
            >
                ...
                >
                    <path
                        d="M5 11.5L6.5 17H17.5L19 11.5L17 6.5H7L5 11.5Z"
                        stroke="white"
                        stroke-width="1.6"
        ...
        const fetchRoutes=async ()=>{
            try {
                if(mapStatus==="arriving"){
                    ...
                    if(pickUpRoute){
                        setRouteToPickUp(pickUpRoute.geometry.coordinates.map(([long, lat]: number[]) => [lat, long]))
                    }

                    if(dropRoute){
                        setRouteToDrop(dropRoute.geometry.coordinates.map(([long, lat]: number[]) => [lat, long]))
                    }
  ...
  return (
    <div className='relative h-full w-full bg-zinc-100'>
          ...
          >
            ...
            {showPickUpMarker && <Marker key="pickup-marker" 
            position={pickUpLocation as any} icon={pickUpIcon}
            />}

            {dropLocation && <Marker key="drop-marker" 
            position={dropLocation as any} icon={dropIcon} 
            />}

            {driverLocation && <Marker key="driver-marker" 
            position={driverLocation as any} icon={driverIcon}
            />}
            ...
            {showDropRoute && (
                <Polyline positions={routeToDrop} pathOptions={{ color: "#0a0a0a", weight: 5, lineCap: "round", lineJoin: "round" }}/>
            )}    
